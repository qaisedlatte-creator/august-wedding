    ...
    <div className="flex flex-col items-center gap-6">
      ...
      >
        <svg viewBox="0 0 144 144" className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle cx="72" cy="72" r="70" fill="none" stroke="#C9A84C" strokeWidth="0.8" strokeDasharray="4 3" opacity="0.45" />
          <circle cx="72" cy="72" r="62" fill="#4A0808" />
          <circle cx="72" cy="72" r="62" fill="url(#waxBody)" />
          <circle cx="72" cy="72" r="58" fill="none" stroke="#C9A84C" strokeWidth="0.9" opacity="0.4" />
        ...
        <span className="relative z-10 font-cormorant tracking-[0.35em] uppercase" style={{ fontSize: '0.58rem', color: 'rgba(201,168,76,0.65)' }}>
          Open
        </span>
      </motion.button>
...
export default function WeddingDetails() {
  ...
  return (
    <section id="details" className="relative py-24 md:py-36 overflow-hidden" style={{ backgroundColor: 'transparent' }}>
      <ConfettiRenderer particles={particles} />

      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }}>

          <motion.p variants={fadeUp} className="font-cormorant tracking-[0.32em] text-xs uppercase mb-5" style={{ color: '#8B6F1E' }}>
            You Are Invited
          </motion.p>

          <motion.h2 variants={fadeUp} className="font-cormorant" style={{ fontSize: 'clamp(2.5rem, 7vw, 5rem)', color: '#3D0A0A', lineHeight: 1, fontWeight: 400 }}>
            Wedding Day
          </motion.h2>
          ...
          <motion.div variants={fadeUp} className="relative">
            {/* Date/time cards — blurred until revealed */}
            <motion.div
              animate={{
            ...
            >
              <div className="relative flex flex-col md:flex-row items-center gap-12 md:gap-0">
                <div className="hidden md:block absolute left-0 right-0 top-1/2 -translate-y-1/2 h-px" style={{ background: 'linear-gradient(to right, transparent 0%, rgba(201,168,76,0.3) 30%, rgba(201,168,76,0.3) 70%, transparent 100%)' }} />

                {/* Date block */}
                <div className="flex-1 relative z-10">
                  <div className="invite-card inline-block md:block p-8 md:p-10">
                    <IconBox><Calendar size={20} color="#B8902A" /></IconBox>
                    <p className="font-inter tracking-[0.28em] text-xs uppercase mb-3" style={{ color: '#8B6F1E', fontWeight: 300 }}>Date</p>
                    <p className="font-playfair italic" style={{ fontSize: 'clamp(1.2rem, 3.5vw, 1.7rem)', color: '#3D0A0A' }}>Sunday</p>
                    <p className="font-cormorant leading-none" style={{ fontSize: 'clamp(3rem, 8vw, 5rem)', color: '#8B1A1A', fontWeight: 400 }}>09</p>
                    <p className="font-playfair tracking-widest" style={{ fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', color: '#3D0A0A' }}>August 2026</p>
                  </div>
                </div>

                <div className="hidden md:flex items-center justify-center z-10 relative">
                  <div className="w-5 h-5 rotate-45" style={{ background: '#C9A84C', boxShadow: '0 0 20px rgba(201,168,76,0.6)' }} />
                </div>
                ...
                <div className="flex-1 relative z-10">
                  <div className="invite-card inline-block md:block p-8 md:p-10">
                    ...
                    <p className="font-inter tracking-[0.28em] text-xs uppercase mb-3" style={{ color: '#8B6F1E', fontWeight: 300 }}>Time</p>
                    <p className="font-playfair italic" style={{ fontSize: 'clamp(2rem, 5.5vw, 3.2rem)', color: '#8B1A1A' }}>11:00 AM</p>
                    <p className="font-playfair italic my-1" style={{ fontSize: '0.9rem', color: 'rgba(61,10,10,0.55)' }}>until</p>
                    <p className="font-playfair italic" style={{ fontSize: 'clamp(2rem, 5.5vw, 3.2rem)', color: '#8B1A1A' }}>2:30 PM</p>
                  </div>
                </div>
              </div>
            </motion.div>

            {/* Wax seal — exits when tapped */}
            <AnimatePresence>
              {!revealed && (
                <motion.div
              ...
              {revealed && (
                ...
                >
                  ...
                  <motion.p
                    ...
                    className="font-playfair italic"
                    style={{ fontSize: '0.9rem', color: 'rgba(61,10,10,0.6)' }}
                  >
